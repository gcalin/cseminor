...
def plot_diffusivity():
    ...
    for i in range(len(filenames)):
        ...
        self_diff[i,:]=plot('Plot of diffusivity '+str(i+1), 'Time', r'$MSD_{Diffusivity}$', lines[0], lines[1:], ['H2O', 'Na', 'Cl'], linear=linearPart)   
    return self_diff
...
def plot_MS_diffusivity():
    ...
    for i in range(len(filenames)):
        ...
        MS_diff[i,:] = plot('Plot of MS diffusivity' + str(i+1), 'Time', r'$MSD_{MS Diffusivity}$', lines[0], lines[1:], ['water-water', 'water-Na', 'Na-Cl', 'water-Cl','Na-Na', 'Cl-Cl'], linear=linearParts)
        #print(MS_diff)
    return MS_diff
...
N_Na = 54
N_Cl = 54
N = N_water + N_Na + N_Cl #number of molecules
...
for i in range(len(self_diff)):
    if (self_diff != None).all():
        ...
        self_diff[i][1] = self_diff[i][1]/N_Na #NaCl
        self_diff[i][2] = self_diff[i][2]/N_Cl #NaCl
